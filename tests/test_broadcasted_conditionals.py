...
def test_conditional_broadcasting(full_cov, white, conditional_type):
    ...
    vars = np.array([sample_conditional_fn(X)[2] for X in Data.SX])

    samples_S12, means_S12, vars_S12 = sample_conditional(Data.SX, feat, kern,
    ...
    samples_S1_S2, means_S1_S2, vars_S1_S2 = sample_conditional(Data.S1_S2_X, feat, kern,
                                                                tf.convert_to_tensor(q_mu),
                                                                q_sqrt=tf.convert_to_tensor(q_sqrt),
                                                                white=white, full_cov=full_cov,
                                                                num_samples=num_samples
    ...
    assert_allclose(vars_S12, vars)
    assert_allclose(means_S1_S2.numpy().reshape(Data.S1 * Data.S2, Data.N, Data.Dy), means)
    if full_cov:
        assert_allclose(vars_S1_S2.numpy().reshape(Data.S1 * Data.S2, Data.Dy, Data.N, Data.N),
                        vars)
    else:
        assert_allclose(vars_S1_S2.numpy().reshape(Data.S1 * Data.S2, Data.N, Data.Dy), vars)
